 ...
 * @author holyong
 * @date 2022-12-20
 * @description 物联网监控控制器
 */
@Api(value = "物联网监控", tags = "物联网监控")
@RestController
@RequestMapping("iot-monitor")
public class IotMonitorController {

    @Autowired
    private IotMonitorService targetService;
    ...
    * @return
    */
    @ApiOperation(value = "获取物联网监控分页列表", notes = "获取物联网监控分页列表")
    @GetMapping(value = "{page}/{limit}")
    public ResponseResult list( @ApiParam(name = "current", value = "当前页码", required = true)
            @PathVariable Long current,
            @ApiParam(name = "limit", value = "每页记录数", required = true)
            @PathVariable Long limit,
            @RequestBody(required = false) IotMonitor entity) {
        Page<IotMonitor> pageParam = new Page<>(current, limit);
        QueryWrapper<IotMonitor> wrapper = new QueryWrapper<>();
    ...
    * 查找所有记录数
    */
    @ApiOperation(value = "获取物联网监控的所有数据")
    @GetMapping("/findAll")
    public ResponseResult findAll() {
        List<IotMonitor> list = targetService.list();
        if (list != null){
     ...
     * 根据ID查找数据
     */
    @ApiOperation(value = "获取物联网监控详情", notes = "根据ID获取物联网监控详情")
    @GetMapping("/get")
    public ResponseResult get(@RequestParam("id") String id) {
        IotMonitor entity = targetService.getById(id);
        if (entity != null){
     ...
     * @return
     */
    @ApiOperation(value = "新增物联网监控", notes = "新增物联网监控")
    @PostMapping("/add")
    public ResponseResult add(@RequestBody IotMonitor entity) {
     ...
     * @return
     */
    @ApiOperation(value = "编辑物联网监控", notes = "编辑物联网监控")
    @PostMapping("/update")
    public ResponseResult update(@RequestBody IotMonitor entity) {
    ...
    * @return
    */
    @ApiOperation(value = "移除物联网监控", notes = "移除物联网监控")
    ...
    * @return
    */
    @ApiOperation(value = "批量移除物联网监控", notes = "批量移除物联网监控 多个参数用,号隔开")
    @ApiResponses({
