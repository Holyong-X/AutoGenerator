 ...
 * @description 村使馆控制器
 */
@Api(value = "村使馆", tags = "村使馆")
@RestController
@RequestMapping("history-museum")
public class HistoryMuseumController {

    @Autowired
    private HistoryMuseumService targetService;
    ...
    * @return
    */
    @ApiOperation(value = "获取村使馆分页列表", notes = "获取村使馆分页列表")
    @GetMapping(value = "{page}/{limit}")
    public ResponseResult list( @ApiParam(name = "current", value = "当前页码", required = true)
            @PathVariable Long current,
            @ApiParam(name = "limit", value = "每页记录数", required = true)
            @PathVariable Long limit,
            @RequestBody(required = false) HistoryMuseum entity) {
        Page<HistoryMuseum> pageParam = new Page<>(current, limit);
        QueryWrapper<HistoryMuseum> wrapper = new QueryWrapper<>();
    ...
    * 查找所有记录数
    */
    @ApiOperation(value = "获取村使馆的所有数据")
    @GetMapping("/findAll")
    public ResponseResult findAll() {
        List<HistoryMuseum> list = targetService.list();
        if (list != null){
     ...
     * 根据ID查找数据
     */
    @ApiOperation(value = "获取村使馆详情", notes = "根据ID获取村使馆详情")
    @GetMapping("/get")
    public ResponseResult get(@RequestParam("id") String id) {
        HistoryMuseum entity = targetService.getById(id);
        if (entity != null){
     ...
     * @return
     */
    @ApiOperation(value = "新增村使馆", notes = "新增村使馆")
    @PostMapping("/add")
    public ResponseResult add(@RequestBody HistoryMuseum entity) {
     ...
     * @return
     */
    @ApiOperation(value = "编辑村使馆", notes = "编辑村使馆")
    @PostMapping("/update")
    public ResponseResult update(@RequestBody HistoryMuseum entity) {
    ...
    * @return
    */
    @ApiOperation(value = "移除村使馆", notes = "移除村使馆")
    ...
    * @return
    */
    @ApiOperation(value = "批量移除村使馆", notes = "批量移除村使馆 多个参数用,号隔开")
    @ApiResponses({
