 ...
 * @author holyong
 * @date 2022-12-20
 * @description 菜园统计监控控制器
 */
@Api(value = "菜园统计监控", tags = "菜园统计监控")
@RestController
@RequestMapping("garden-monitor")
public class GardenMonitorController {

    @Autowired
    private GardenMonitorService targetService;
    ...
    * @return
    */
    @ApiOperation(value = "获取菜园统计监控分页列表", notes = "获取菜园统计监控分页列表")
    @GetMapping(value = "{page}/{limit}")
    public ResponseResult list( @ApiParam(name = "current", value = "当前页码", required = true)
            @PathVariable Long current,
            @ApiParam(name = "limit", value = "每页记录数", required = true)
            @PathVariable Long limit,
            @RequestBody(required = false) GardenMonitor entity) {
        Page<GardenMonitor> pageParam = new Page<>(current, limit);
        QueryWrapper<GardenMonitor> wrapper = new QueryWrapper<>();
    ...
    * 查找所有记录数
    */
    @ApiOperation(value = "获取菜园统计监控的所有数据")
    @GetMapping("/findAll")
    public ResponseResult findAll() {
        List<GardenMonitor> list = targetService.list();
        if (list != null){
     ...
     * 根据ID查找数据
     */
    @ApiOperation(value = "获取菜园统计监控详情", notes = "根据ID获取菜园统计监控详情")
    @GetMapping("/get")
    public ResponseResult get(@RequestParam("id") String id) {
        GardenMonitor entity = targetService.getById(id);
        if (entity != null){
     ...
     * @return
     */
    @ApiOperation(value = "新增菜园统计监控", notes = "新增菜园统计监控")
    @PostMapping("/add")
    public ResponseResult add(@RequestBody GardenMonitor entity) {
     ...
     * @return
     */
    @ApiOperation(value = "编辑菜园统计监控", notes = "编辑菜园统计监控")
    @PostMapping("/update")
    public ResponseResult update(@RequestBody GardenMonitor entity) {
    ...
    * @return
    */
    @ApiOperation(value = "移除菜园统计监控", notes = "移除菜园统计监控")
    ...
    * @return
    */
    @ApiOperation(value = "批量移除菜园统计监控", notes = "批量移除菜园统计监控 多个参数用,号隔开")
    @ApiResponses({
